...
function App() {
  ...
  // hiding search bar 
  const searchBarHiddenRoutes = [ '/profile/login', '/profile/register', '/bookmarks'];
  const isSearchBarHidden = searchBarHiddenRoutes.includes(location.pathname);

  // css 
  const containerstyle = 'bg-leanBlue relative h-screen w-screen overflow-y-scroll flex flex-col overflow-x-hidden items-center justify-evenly text-white lg:flex-row scrollbar-corner-transparent scrollbar-thin  scrollbar-thumb-cyan-500 scrollbar-track-transparent overflow-scroll py-5';
